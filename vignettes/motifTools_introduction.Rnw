...
\title{motifTools: analysis and intepretation of sequence motifs}
\maketitle
...
Motif ranges are stored as \texttt{RangedData} objects (\texttt{IRanges} package). This allows to include metadata, like p-value, score and other output from the search process.
